...
export function startRiftReliquaryEvent(dimension,center,type){
  ...
  let otherStates=[],preDragonStates=[];
  for(const [key,target] of [["lb:mythic_event_states_v1",otherStates],["lb:pre_dragon_event_states_v1",preDragonStates]]){
  ...
  if(states.length+otherStates.length+preDragonStates.length>=4)return false;
  ...
  for(const s of states)if(s.dimension===key&&distSq(s.center,site)<80*80)return false;
  for(const s of otherStates)if(s.dimension===key&&distSq(s.center,site)<80*80)return false;
  for(const s of preDragonStates)if(s.dimension===key&&distSq(s.center,site)<80*80)return false;
  states.push({id:nextId(),type,dimension:key,center:site,stage:0,elapsed:0,keyMask:0,spentMask:0,disarms:0});
